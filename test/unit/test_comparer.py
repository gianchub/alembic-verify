...
class TestCompareCallsChain(object):
    ...
    def _get_inspectors_mock(self):
        with patch('alembicverify.comparer._get_inspectors') as m:
            m.return_value = [
    ...
    def _get_tables_data_mock(self):
        with patch('alembicverify.comparer._get_tables_data') as m:
            yield m

    @pytest.yield_fixture
    def _compile_errors_mock(self):
        with patch('alembicverify.comparer._compile_errors') as m:
    ...
    def _get_tables_info_mock(self):
        with patch('alembicverify.comparer._get_tables_info') as m:
...
class TestCompareInternals(object):
    ...
    def _get_table_data_mock(self):
        with patch('alembicverify.comparer._get_table_data') as m:
            yield m

    @pytest.yield_fixture
    def _diff_dicts_mock(self):
        with patch('alembicverify.comparer._diff_dicts') as m:
            yield m

    @pytest.yield_fixture
    def _get_foreign_keys_mock(self):
        with patch('alembicverify.comparer._get_foreign_keys') as m:
            yield m

    @pytest.yield_fixture
    def _get_primary_keys_mock(self):
        with patch('alembicverify.comparer._get_primary_keys') as m:
            yield m

    @pytest.yield_fixture
    def _get_indexes_mock(self):
        with patch('alembicverify.comparer._get_indexes') as m:
            yield m

    @pytest.yield_fixture
    def _get_columns_mock(self):
        with patch('alembicverify.comparer._get_columns') as m:
            yield m

    @pytest.yield_fixture
    def _process_types_mock(self):
        with patch('alembicverify.comparer._process_types') as m:
            yield m

    @pytest.yield_fixture
    def _process_type_mock(self):
        with patch('alembicverify.comparer._process_type') as m:
            yield m

    @pytest.yield_fixture
    def _get_foreign_keys_info_mock(self):
        with patch('alembicverify.comparer._get_foreign_keys_info') as m:
            yield m

    @pytest.yield_fixture
    def _get_primary_keys_info_mock(self):
        with patch('alembicverify.comparer._get_primary_keys_info') as m:
            yield m

    @pytest.yield_fixture
    def _get_indexes_info_mock(self):
        with patch('alembicverify.comparer._get_indexes_info') as m:
            yield m

    @pytest.yield_fixture
    def _get_columns_info_mock(self):
        with patch('alembicverify.comparer._get_columns_info') as m:
            yield m
